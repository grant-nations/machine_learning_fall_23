...
class ThreeLayerNN:
    ...
    def step(self, x: npt.ArrayLike, y: int, verbose: bool = False) -> Tuple[float, npt.ArrayLike]:
        ...
        dprediction = layer_3 - y  # (1,)
        if verbose:
            print(f"dprediction: {dprediction}")
        ...
        dlayer_2 = dlayer_2_sig @ self.w2.T  # (H2,) @ (H2, H1) -> (H1,)
        if verbose:
            print(f"dlayer_2: {dlayer_2}")

        dw2 = np.outer(layer_1_sig, dlayer_2_sig)  # (H1,) X (H2,) -> (H1, H2)
