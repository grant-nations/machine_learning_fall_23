...
for i in range(num_predictors):
    progress_spinner("generating random forests ensembles...", num_predictors, i + 1)
    ...
    ensemble = random_forests.train(x_samples, y_samples, attributes_proc, num_trees, feature_subset_size)
...
# do it all over again for random forests

for i, (_x, _y) in enumerate(zip(x_test_proc, y_test_proc)):
    progress_spinner("estimating random forests squared error...", len(x_test_proc), i + 1)
    predictions = [random_forests.predict(_x, attributes_proc, e) for e in ensembles]
